...
public class OrderPaymentHistoryMapper {
    ...
    public OrderPaymentHistoryMapper(BusinessAndBranchMapper businessAndBranchMapper, UserServiceImpl userService, OrderRepository orderRepository) {
    ...
    public OrderPaymentHistory toEntity(OrderPaymentHistoryDto detailDto){
        return OrderPaymentHistory.builder()
                ...
                .order(orderRepository.findById(detailDto.getPaymentHistory().getId())
                        .orElseThrow(() -> new RecordNotFoundException("Payment history not found")))
                .build();
    }
